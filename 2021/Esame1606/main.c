...
//chiudo in scrittura tutte le pipe eccetto ((n+1) % N) e in lettura 
//tutte eccetto n
...
write(piped[(1+n) % N][1],tutteLinee,N * sizeof(L));

if ( n == N - 1){

for ( int k = 0 ; k < N ; ++k){
printf("stampo il vettore %s \n",tutteLinee[k]);
...
write(piped[0][1],tutteLinee,N * sizeof(L));
